...
# This function is used to compute the cosine similarity between two vectors. Cosine similarity is useful in this case since we are using it to create a graph of linked records.
...
# This function after we train a word2vec model to learn vectors for documents we use it to retreive the most simlilar documents to the input document. Here similarity entails that the two records might refere to the same entity
...
# This function is used to create a tagged document object for the doc2vec model. However I ended up not using it because word2vec showed better results than doc2vec
def tagged_document(list_of_list_of_words):
    for i, list_of_words in enumerate(list_of_list_of_words):
        yield gensim.models.doc2vec.TaggedDocument(list_of_words, [i])

# This funciton is used to visualize the learned 128 dimnetional vector for each record. It project each vector using the tSNE algorthm to a 2 dimentaionl vecotr that can be easily visualized.
...
# This function is used to evaluate the learned word2vec or doc2vec model using the true id or ground truth provided in the data file. The heuristic used simplu counts the number of records that are supposed to be clustered with the target record in the top ten similar vector records.
def evaluate_model(model, embedding_size, docs):
    k = 10
    average_accuracy = 0
    loop_count = 0
...
# Infer vector for a document. Simply done by computing the average of the vectors for each word in the record learned by word2vec
...
if __name__ == '__main__':
    ...
    # Training the Doc2Vec model. However I ended up training a Word2Vec model instea. The skip gram variation of the Word2Vec model is very suitable for capturing similarity between records automatically.
    # data_for_training = list(tagged_document(docs))
    # model = gensim.models.doc2vec.Doc2Vec(dm=1, vector_size=48, window=7, alpha=0.001, min_alpha=0.0001, min_count=2,
    #                                       max_vocab_size=None, sample=0, workers=1, epochs=50, hs=0, negative=15,
    #                                       ns_exponent=0.75, dm_mean=1, dm_concat=0, dm_tag_count=1, dbow_words=0)
    # model.build_vocab(data_for_training)
    # model.train(data_for_training, total_examples=model.corpus_count, epochs=model.epochs)

    # Training Word2Vec is mode suitable I think for this task as it uses the skip gram model with negative sampling which trains a neural network to endocde each unique word in the corpus to a unique vector. The trained vectors end up close to each other in the euclidean space if the records are similai or in other words if they should exist in the same cluster.
    embedding_size = 128
    # alpha = 0.05
    # window = 20
    # workers = 1
    ...
    # sorted_vocab = 1
    # min_count = 0
    # model = gensim.models.word2vec.Word2Vec(sentences=docs, size=embedding_size, alpha=alpha, window=window,
    ...
    # Simple test
    test_doc = ['61', 'laverne', 'l', 'delinois', '1439', 'sara', 'ct', 'allen', 'tx', '75002', '972', '412', '0632']
    c = test_doc[0]
    current_cluster_count = get_number_of_occurances_in_list(c, ground_truth_clusters)
    test_doc_vec = infer_vector(model, embedding_size, test_doc)
    top_k_similar_docs = top_k_similar_documents(model, embedding_size, test_doc, docs, 10)
    similar_doc_count = 0
    for s in top_k_similar_docs:
        similar_doc = s[0]
        similar_doc_type = s[0][0]
    ...
    # visualize_embedding(model, embedding_size, docs)

    # Evaluate
    average_accuracy = evaluate_model(model, embedding_size, docs)
    ...
    # print(len(docs))
    # for d in tqdm(docs):
    #     top_similar_doc_for_graph = top_k_similar_documents(model, embedding_size, d, docs, K)
